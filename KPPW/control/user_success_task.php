...
if($uid){
	...
	$where = ' uid = '.$uid.' and  task_status = 7 ';
	

	if($slt_task_type){
		$where.=' and model_id='.intval($slt_task_type);
	}
	...
	if(intval($txt_start_time)&&intval($txt_end_time)){
		if(intval($txt_start_time)>=intval($txt_end_time)){
			...
			$where.= ' and a.start_time = '.intval($txt_start_time);
		}else{
			$where.= ' and a.start_time >= '.intval($txt_start_time).' and a.start_time <= '.intval($txt_end_time);
		}
	}
