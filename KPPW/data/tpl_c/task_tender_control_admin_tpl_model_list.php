<?php Template_Class::subtplcheck('task/tender/control/admin/tpl/model_list', '1303811300', 'task/tender/control/admin/tpl/model_list');?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
<option value="2" <?php if($status==2) { ?>selected="selected"<?php } ?>>投标中</option>
<option value="3" <?php if($status==3) { ?>selected="selected"<?php } ?>>进行中</option>
...
<input type="submit" name="sbt_search" value="查询" class="input_but"/>
    ...
    <div class="div_tit_a">
 	...
 	<a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=1" <?php if($status=='1') { ?>style="color:red"<?php } ?>>未审核</a> | 
 	<a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=2" <?php if($status=='2') { ?>style="color:red"<?php } ?>>招标中</a> | 
 	<a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=3" <?php if($status=='3') { ?>style="color:red"<?php } ?>>进行中</a> | 
  ...
  <div id="change">
          ...
          <tr>
            <th width="65" align="left">ID( <a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=<?=$status?>&txt_task_id=<?=$txt_task_id?>&slt_indus_id=<?=$slt_indus_id?>&txt_leftday=<?=$txt_leftday?>&txt_lefthour=<?=$txt_lefthour?>&order=1"><img src="tpl/img/m_up.gif" /></a> / <a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=<?=$status?>&txt_task_id=<?=$txt_task_id?>&slt_indus_id=<?=$slt_indus_id?>&txt_leftday=<?=$txt_leftday?>&txt_lefthour=<?=$txt_lefthour?>&order=2"><img src="tpl/img/m_down.gif" /></a> )</th>
            <th width="25%" class="motif">任务标题</th>
<th>任务状态</th>
<th>所属行业</th>
<th>发布者</th>
<th>发布时间( <a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=<?=$status?>&txt_task_id=<?=$txt_task_id?>&slt_indus_id=<?=$slt_indus_id?>&txt_leftday=<?=$txt_leftday?>&txt_lefthour=<?=$txt_lefthour?>&order=3"><img src="tpl/img/m_up.gif" /></a> / <a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=<?=$status?>&txt_task_id=<?=$txt_task_id?>&slt_indus_id=<?=$slt_indus_id?>&txt_leftday=<?=$txt_leftday?>&txt_lefthour=<?=$txt_lefthour?>&order=4"><img src="tpl/img/m_down.gif" /></a> )</th>
<th>过期时间( <a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=<?=$status?>&txt_task_id=<?=$txt_task_id?>&slt_indus_id=<?=$slt_indus_id?>&txt_leftday=<?=$txt_leftday?>&txt_lefthour=<?=$txt_lefthour?>&order=5"><img src="tpl/img/m_up.gif" /></a> / <a href="index.php?do=model&model_id=<?=$model_id?>&view=list&status=<?=$status?>&txt_task_id=<?=$txt_task_id?>&slt_indus_id=<?=$slt_indus_id?>&txt_leftday=<?=$txt_leftday?>&txt_lefthour=<?=$txt_lefthour?>&order=6"><img src="tpl/img/m_down.gif" /></a> )</th>
<?php if($status==1) { ?>
<th width="55">审核</th>
<?php } ?>
<?php if($status==7) { ?>
...
<td>
<?php if($value['task_status']==0) { ?>未付款<?php } ?>
<?php if($value['task_status']==1) { ?>待审核<?php } ?>
<?php if($value['task_status']==2) { ?>投标中<?php } ?>
<?php if($value['task_status']==3) { ?>进行中<?php } ?>
