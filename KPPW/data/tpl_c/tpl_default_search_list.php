<?php Template_Class::subtplcheck('tpl/default/search_list', '1303712470', 'tpl/default/search_list');?><?php $page_title ='搜索 - '.$_K['html_title'] ?><?php if($inajax) { ?>
<?php ob_end_clean();
ob_start();
@header("Expires: -1");
...
<option value="6" <?php if($slt_order=='6') { ?>selected="selected"<?php } ?>>价格 从高到低</option>
<option value="5" <?php if($slt_order=='5') { ?>selected="selected"<?php } ?>>价格 从低到高</option>
<option value="4" <?php if($slt_order=='4') { ?>selected="selected"<?php } ?>>剩余时间 从多到少</option>
<option value="3" <?php if($slt_order=='3') { ?>selected="selected"<?php } ?>>剩余时间 从少到多</option>
</select>　
...
<?php } ?>
                    ...
                    <td>
                    	<?php echo date('Y-m-d',$value[start_time]); ?> — <?php echo date('Y-m-d',$value[end_time]); ?><p>
                    		进行中 剩余时间: 
