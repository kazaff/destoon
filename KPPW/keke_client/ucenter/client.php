...
function uc_api_post($module, $action, $arg = array()) {
	...
	return uc_fopen2($_UC['uc_api' ].'/index.php', 500000, $postdata, '', TRUE, $_UC['uc_ip'], 20);
...
function uc_api_requestdata($module, $action, $arg='', $extra='') {
	...
	$post = "m=$module&a=$action&inajax=2&release=".UC_CLIENT_RELEASE."&input=$input&appid=".$_UC['uc_appid'].$extra;
	return $post;
}

function uc_api_url($module, $action, $arg='', $extra='') {
	global $_UC;
	$url = $_UC['uc_api'].'/index.php?'.uc_api_requestdata($module, $action, $arg, $extra);
	return $url;
}

function uc_api_input($data) {
	global $_UC;
	$s = urlencode(uc_authcode($data.'&agent='.md5($_SERVER['HTTP_USER_AGENT'])."&time=".time(), 'ENCODE', $_UC['uc_key']));
	return $s;
}
...
function uc_app_ls() {
	...
	return $_UC['uc_connect'] == 'mysql' ? $return : uc_unserialize($return);
}
...
function uc_avatar($uid, $type = 'virtual', $returnhtml = 1) {
	...
	$uc_avatarflash = $_UC['uc_api'].'/images/camera.swf?inajax=1&appid='.$_UC['uc_appid'].'&input='.$uc_input.'&agent='.md5($_SERVER['HTTP_USER_AGENT']).'&ucapi='.urlencode($_UC['uc_api']).'&avatartype='.$type;
	if($returnhtml) {
...
function uc_check_avatar($uid, $size = 'middle', $type = 'virtual') {
	$url = $_UC['uc_api']."/avatar.php?uid=$uid&size=$size&type=$type&check_file_exists=1";
