...
abstract class Time_base_class {
	...
	abstract public function validmarkstatus();
	
	protected function getuserinfo($uid){
		if (!$this->_a_uinfo[$uid])
		{
			$this->_a_uinfo[$uid] = Func_comm_class::getuserinfo($uid);
		}
	...
	protected function excutecommon(){
		...
		if ($this->_a_fina_arr){
			$fina_sql = "insert into ".TABLEPRE."witkey_finance (fina_type,fina_narrate,uid,username,task_id,fina_cash,user_balance,fina_credit,user_credit,fina_time)  values ";
			$i =0;
			foreach ($this->_a_fina_arr as $fina){
				...
				$fina_sql .="('".($fina['fina_type']+0)."','".($fina['fina_narrate']+0)."','".($fina['uid']+0)."','{$fina['username']}','".($fina['task_id']+0)."','".($fina['fina_cash']+0)."','".($fina['user_balance']+0)."','".($fina['fina_credit']+0)."','".($fina['user_credit']+0)."','".($fina['fina_time']+0)."')";
			}
