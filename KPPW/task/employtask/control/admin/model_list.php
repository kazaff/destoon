		...
		if (count ( $keyids )) {
			...
			elseif ($sbt_action == "批量恢复")
			{
				...
				foreach ($task_arr as $task)
				{
					...
					$task_obj->edit_keke_witkey_task();
					Func_comm_class::notify_user("任务恢复通知",'您的任务 <b>'.$task['task_title'].'</b> 的冻结状态已被恢复，',$task['uid'],$task['username']);
				}
...
$temp_arr = array ();
$indus_option_arr = Cache_ext_Class::getIndustryList();
