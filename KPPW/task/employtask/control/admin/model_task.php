...
if ($op == 'del')
{
	...
	Func_comm_class::notify_user("任务删除通知","您的任务 <b>{$task_arr[0]['task_title']}</b> 被删除，如有疑问请联系本站客服",$task_arr[0]['uid'],$task_arr[0]['username']);
...
if ($op == 'restore')
{
	...
	Func_comm_class::notify_user("任务恢复通知","您的任务 <b>{$task_arr[0]['task_title']}</b> 的冻结状态已被恢复，",$task_arr[0]['uid'],$task_arr[0]['username']);
	if ($task_arr[0]['model_id']==1){
