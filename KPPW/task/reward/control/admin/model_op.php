...
if ($op == 'del')
{
	...
	Func_comm_class::notify_user("任务删除提示","您的任务 <b>{$task_info['task_title']}</b> 被删除，如有疑问请联系本站客服",$task_info['uid'],$task_info['username']);
...
if ($op == 'restore')
{
	...
	Func_comm_class::notify_user("任务冻结通知","您的任务 <b>{$task_info['task_title']}</b> 的冻结状态已被恢复，",$task_info['uid'],$task_info['username']);
	if ($task_info['model_id']==1){
