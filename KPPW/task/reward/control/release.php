...
if(Func_comm_class::submitcheck ('formhash')){
	...
	if($slt_task_mode==3)
	{
		...
		$task_obj->setWork_count(intval($gj_num));
		
		$task_obj->setSingle_cash(floatval($gj_money));
	}
	...
	$task_obj->setProm_count($prom_cash);
	...
	if($res){
		...
		switch ($hdn_paystatus) {
			case 'confirm':
				...
				if($xs_config['vip_recommend']==1 && $audit_status != 1){
					
	                $space_obj = new Keke_witkey_space_class();
	                $space_obj->setWhere(" isvip = 1 and indus_id ='$slt_indus_id' and username != '$username'");			
					...
					if(count($user_arr))
					{
					 $js .= '<script>
					       $(function(){
				           showWindow("invite","index.php?do=release&model_id='.$model_id.'&ajax=invite&indus_id='.$slt_indus_id.'&task_id='.$res.'&task_title='.$txt_task_title.'");
			               })
					 </script>';
					  $js.='<p>
					...
					Func_comm_class::showmessage('任务发布提示','index.php?do=user',9999999,$js);
					}else{
						
						Func_comm_class::showmessage('任务发布提示','index.php?do=task_list&model_id='.$model_id,2,'任务发布成功!');
					} 
				   ...
				   Func_comm_class::showmessage('任务发布提示','index.php?do=user',3,'悬赏任务发布成功！');	
				}
	...
	switch ($ajax) {
    	...
    	case 'sms':
    		...
    		{ 	
    		    ...
    		    $sms_conent = $username.'于'.date('Y年-m月-d日',time()).'发布了悬赏任务<< <a href="index.php?do=task&task_id='.$task_id.'" target="_blank" >'.$task_title.' </a>  >> 特邀请您参与投标!'; 
    		    ...
    		    for ($i=0;$i<count($uid_arr);$i++)
 				{
    		      ...
    		      $sql .= "($uid_arr[$i],'$username_arr[$i]','悬赏任务邀请通知!','$sms_conent',".time().")";
 				}
    		...
    		die();
    	break;
    	//delete attachment in database and io
    	case 'del_att':
