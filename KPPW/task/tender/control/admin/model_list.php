	  ...
	  $ids =$keyids?implode(',',$keyids):Func_comm_class::adminshowmessage("您没有选择项","index.php?do=model&model_id=$model_id&view=list");
		...
		if (count ( $keyids )) {
			...
			elseif ($sbt_action == "批量审批"){
				...
				$task_rule_config = Cache_ext_Class::getConfigRule('day');
				foreach ($task_arr as $task)
				{
					...
					db_factory::execute("update ".TABLEPRE."witkey_finance set site_profit='{$task['task_cash']}' where task_id={$task['task_id']} and fina_narrate = 4");
				}	
				Func_comm_class::adminSystemLog("审批任务$ids");
			}
			elseif ($sbt_action == "批量冻结"){
				$setstatus = " &status=6 ";
				//$setstatus = "&status in(3,6)";
				...
				foreach ($task_arr as $task)
				{
					...
					if ($task['task_status']==2){
						//Func_comm_class::adminshowmessage('操作失败!不能冻结招标中的任务!',"index.php?do=model&model_id=$model_id&view=list".$setstatus);
						continue;
					}
			...
			elseif ($sbt_action == "批量恢复")
			{
				...
				foreach ($task_arr as $task)
				{
					...
					$task_obj->edit_keke_witkey_task();
					Func_comm_class::notify_user("任务回复通知",'您的任务 <b>'.$task['task_title'].'</b> 的冻结状态已被恢复。',$task['uid'],$task['username']);
				}
				Func_comm_class::adminSystemLog("恢复任务$ids");
			}
			
			
			Func_comm_class::adminshowmessage('操作成功',"index.php?do=model&model_id=$model_id&view=list".$setstatus);
		}
	 } 
