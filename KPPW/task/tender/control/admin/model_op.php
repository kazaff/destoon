...
if ($op == 'setstatus1')
{
	...
	if ($task_info['task_status']!=1){
		Func_comm_class::adminshowmessage("该任务并不在待审核状态","index.php?do=model&model_id=2&view=list&status=0");
	}
	$task_obj->setWhere("task_id='{$task_id}'");
	...
	if ($task_info['model_id']==1){
		...
		db_factory::execute("update ".TABLEPRE."witkey_finance set site_profit='".$task_info['task_cash']."' where task_id={$task_info['task_id']} and fina_narrate = 4");
