...
if ($op == 'setstatus5'){
	...
	if ($task_arr[0]['is_prom'])
	{
		...
		db_factory::execute("update ".TABLEPRE."witkey_space set balance=balance+{$task_arr[0]['prom_cash']},credit=credit+{$task_arr[0]['prom_credit']} where uid='{$task_arr[0]['uid']}'");
...
if ($op == 'restore')
{
	...
	Func_comm_class::notify_user("任务恢复通知","您的任务 <b><a href=\"index.php?do=task&task_id={$task_arr[0]['task_id']}\">{$task_arr[0]['task_title']}</a></b> 的冻结状态已被恢复，",$task_arr[0]['uid'],$task_arr[0]['username']);
	if ($task_arr[0]['model_id']==1){
