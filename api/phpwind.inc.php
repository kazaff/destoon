...
if($action == 'login') {
	...
	$URI = $jumpurl.'/passport_client.php?action=login&userdb='.rawurlencode($userdb_encode).'&forward='.rawurlencode($forward).'&verify='.rawurlencode($verify);
} else if($action == 'logout') {
	$verify = md5('quit'.$userdb_encode.$forward.$MOD['passport_key']);
    $URI = $jumpurl.'/passport_client.php?action=quit&userdb='.rawurlencode($userdb_encode).'&forward='.rawurlencode($forward).'&verify='.rawurlencode($verify);
}
