...
switch($action) {
	...
	case 'synlogin':
		...
		$user = $db->get_one("SELECT userid,password,username,passport,groupid,message FROM {$DT_PRE}member WHERE passport='$username'");
		if(!$user || $user['groupid'] == 2 || $user['groupid'] == 4) exit('-1');
		$cookietime = $DT_TIME + ($cookietime ? $cookietime : 86400);
		$destoon_auth = encrypt($user['userid']."\t".$user['username']."\t".$user['groupid']."\t".$user['password'], md5(DT_KEY.$_SERVER['HTTP_USER_AGENT']));
		ob_clean() ;
