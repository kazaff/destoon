...
//跳转页面函数,defend_reload表示后台设置的最短刷新时间间隔
...
function encrypt($txt, $key = '') {
	...
	return base64_encode(kecrypt($tmp, $key));
}
...
function kecrypt($txt, $encrypt_key) {
...
function send_message($touser, $title, $content, $typeid = 4, $fromuser = '') {
	...
	if(check_name($touser) && $title && $content) {
		...
		$r = $db->get_one("SELECT username,black FROM {$db->pre}member WHERE username='$touser'");
		if($r) {
			if($r['black'] && $typeid != 4) {
...
function banword($WORD, $string, $extend = true) {
	...
	foreach($WORD as $v) {
		if($v['deny'] && $extend) {
			if(preg_match("/".$v['replacefrom']."/i", $string)) dalert(lang('include->msg_word_ban').'->('.$v['replacefrom'].')');
		} else {
	...
	if($enable) {
		if($DT['captcha_cn']) {
			...
			if(!preg_match("/^[0-9a-z]{4,}$/i", $captcha)) {
				$msg = lang('include->captcha_missed');		//captcha_missed:请填写验证码
				return $return ? $msg : message($msg);
			}
		}
...
function listpages($moduleid, $catid, $total, $page = 1, $perpage = 20, $category = array(), $step = 2) {
	...
	$total = ceil($total/$perpage);
	if($page < 1 || $page > $total) $page = 1;
	$home_url = $M['linkurl'].listurl($moduleid, $catid, 0, $C, $M);
...
function imgurl($imgurl = '', $absurl = 0) {
	if(!$imgurl) $imgurl = DT_SKIN.'image/nopic.gif';
...
function ad($id, $cid = 0, $kw = '', $tid = 0) {
	if($tid) {
		if($kw) {
			$file = 'ad_t'.$tid.'_m'.$id.'_k'.urlencode($kw).'.htm';
		} else if($cid) {
			$file = 'ad_t'.$tid.'_m'.$id.'_c'.$cid.'.htm';
		} else {
			$file = 'ad_t'.$tid.'_m'.$id.'.htm';
		}
	} else {
		$file = 'ad_'.$id.'.htm';
	}
	load(is_file(DT_CACHE.'/htm/'.$file) ? $file : $file.'.htm');
}
...
function check_name($username) {
	if(strpos($username, '__') !== false) return false; 
