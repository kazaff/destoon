...
function dstyle($name, $value = '') {
	...
	$style .= '<input type="hidden" name="'.$name.'" id="color_input_'.$destoon_style_id.'" value="'.$color.'"/><img src="'.DT_SKIN.'image/color.gif" width="21" height="18" align="absmiddle" id="color_img_'.$destoon_style_id.'" style="cursor:pointer;background:'.$color.'" onclick="color_show('.$destoon_style_id.', $(\'color_input_'.$destoon_style_id.'\').value, this);"/>';
	return $style;
}
...
function dcalendar($name, $value = '', $sep = '-') {
	...
	$calendar .= '<input type="text" name="'.$name.'" id="'.$id.'" value="'.$value.'" size="10" onfocus="ca_show(\''.$id.'\', this, \''.$sep.'\');" readonly ondblclick="this.value=\'\';"/> <img src="'.DT_SKIN.'image/calendar.gif" align="absmiddle" onclick="ca_show(\''.$id.'\', this, \''.$sep.'\');" style="cursor:pointer;"/>';
	return $calendar;
}
...
function delete_upload($file, $userid) {
	...
	if($DT['uploadlog']) $db->query("DELETE FROM {$db->pre}upload WHERE item='".md5($fileurl)."'");
}
...
function clear_upload($content = '', $itemid = 0) {
	...
	if($update) $db->query("UPDATE {$db->pre}upload SET itemid=$itemid WHERE item IN (".implode(',', $update).")");
