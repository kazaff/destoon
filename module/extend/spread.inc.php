...
if($action == 'list') {
	if($MOD['spread_list'] && $kw && in_array($mid, array(4, 5, 6))) {
		...
		$i = $j = 0;
		$lists = array();
		$result = $db->query("SELECT * FROM {$DT_PRE}spread WHERE $condition ORDER BY fromtime DESC,price DESC,itemid DESC LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
			...
			if($j == 0) $i = $r['fromtime'];
			$r['bg'] = $i == $r['fromtime'] ? 0 : 1;
			if($i != $r['fromtime']) $i = $r['fromtime'];
			$j++;
			$lists[] = $r;
		}
