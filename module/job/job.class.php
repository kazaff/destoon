...
class job {
    ...
    function job($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','areaid','level','title','style','fee','introduce','department','total','minsalary','maxsalary','type','gender','marriage','education','minage','maxage','experience','status','hits','username','truename','telephone', 'mobile','address','email','msn','qq','ali','skype','totime','editor','addtime','adddate','edittime','editdate','ip','template','linkurl','filepath','note','company');
    }
	...
	function add($post) {
		...
		if($post['status'] == 3 && $post['username'] && $MOD['credit_add']) {
			...
			credit_record($post['username'], $MOD['credit_add'], 'system', lang('my->credit_record_job_add'), 'ID:'.$this->itemid);
		}
		...
		if(is_array($itemid)) {
			...
			if($all) {
				...
				if($r['username'] && $MOD['credit_del']) {
					...
					credit_record($r['username'], -$MOD['credit_del'], 'system', lang('my->credit_record_job_del'), 'ID:'.$this->itemid);
				}
			}
		}
	}
