...
if($action == 'add') {
	...
	if($kw && in_array($mid, array(4, 5, 6))) {
		...
		$spread_max = $_MOD['spread_max'] ? $_MOD['spread_max'] : 10;
		$currency = $_MOD['spread_currency'];
		...
		if($r) message($L['spread_msg_buy'], $spreadurl);
		isset($mid) or $mid = 5;
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$DT_PRE}spread WHERE mid=$mid AND status=3 AND word='$kw' AND fromtime>=$next_time");
		if($r['num'] > $spread_max) message(lang($L['spread_msg_over'], array($kw)), $spreadurl);
		$p = $db->get_one("SELECT * FROM {$DT_PRE}spread_price WHERE word='$kw'");
		if($mid == 4) {
			$price = $p['company_price'] ? $p['company_price'] : $_MOD['spread_company_price'];
		} else if($mid == 5) {
			$price = $p['sell_price'] ? $p['sell_price'] : $_MOD['spread_sell_price'];
		} else if($mid == 6) {
			$price = $p['buy_price'] ? $p['buy_price'] : $_MOD['spread_buy_price'];
		} else {
			dheader($spreadurl);
		}
		$step = $_MOD['spread_step'];
		$month = $_MOD['spread_month'] ? $_MOD['spread_month'] : 1;
		if($submit) {
