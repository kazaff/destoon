...
class special {
    ...
    function special($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','level','title','style','introduce','thumb','banner','cfg_photo','cfg_video','cfg_type','seo_title','seo_keywords','seo_description','tag','status','hits','username','addtime', 'editor','edittime','ip','template','islink','linkurl','filepath','domain','note');
    }
	...
	function update($itemid, $item = array(), $content = '') {
		...
		$item['itemid'] = $itemid;
		$linkurl = $item['domain'] ? $item['domain'] : ($item['islink'] ? $item['linkurl'] : itemurl($item));
